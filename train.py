...
def web_main():
    ...
    adj_train, val_edges, val_edges_false, test_edges, test_edges_false = mask_test_edges(adj)
    adj = adj_train
    ...
    d = ['sigmoid', 'manhattan', 'euclidean', 'sqeuclidean', 'chi-squared', 'cosine', 'linear', 'poly', 'rbf']

    d2 = ['manhattan', 'euclidean', 'sqeuclidean', 'chi-squared', 'cosine']
    d3 = ['linear', 'poly', 'rbf']


    mat = np.zeros(((adj.shape[0]), (adj.shape[0])), dtype=np.float32)


    def sigmoid(x):
            return 1 / (1 + np.exp(-x))

    def computing_logits(itr, sim, emb=None):
        if sim == 'sigmoid':
            adj_rec = torch.sigmoid(torch.matmul(emb, emb.t()))
            ...
            for i in range(f.shape[0]):
                for j in range(f.shape[1]): #print([list(f[i])])
                    if d[k] in d2:
                        m = pdist2(np.array([list(f[i])]), np.array([list(f[j])]), str(sim))
                    elif d[k] in d3:
                        m = pairwise_kernels([list(f[i])], [list(f[j])], metric=sim)
                    #print(type(d[k]))
                    mat[i][j] = m[0][0]

            # if(np.isnan(mat).any()):
            adj_rec = np.nan_to_num(mat) # for nan values 
            adj_rec = torch.sigmoid(torch.from_numpy(adj_rec))            
        return adj_rec
    ...
    def get_scores(edges_pos, edges_neg, adj_rec):
        ...
        roc_score = roc_auc_score(labels_all, preds_all)
        ap_score = average_precision_score(labels_all, preds_all)
        f_score = f1_score(labels_all, (preds_all).round())

        return roc_score, ap_score, f_score
    ...
    for i in range(len(d)):
        ...
        for epoch in range(args.epochs):
            ...
            emb = vgae_model.encoder(graph, features)

            logits = computing_logits(i, d[i], emb)
            ...
            val_roc, val_ap, _ = get_scores(val_edges, val_edges_false, logits)
        ...
        test_roc, test_ap, f_ = get_scores(test_edges, test_edges_false, logits)
        print("End of training!", "test_roc=", "{:.5f}".format(test_roc), "test_ap=", "{:.5f}".format(test_ap), "f1_score=", "{:.5f}".format(f_))
    # roc_means.append(test_roc)
    # ap_means.append(test_ap)
